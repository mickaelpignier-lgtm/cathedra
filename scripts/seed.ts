...
import { coreStadiums } from "./seed-data-core";
import { extraStadiums } from "./seed-data-extra";
...
async function main() {
  ...
  const all: StadiumSeed[] = [...coreStadiums, ...extraStadiums];
  const slugs = new Set<string>();
  for (const s of all) {
    if (slugs.has(s.slug)) {
      throw new Error(`Duplicate slug in seed data: ${s.slug}`);
    }
    slugs.add(s.slug);
  }

  console.log(`Seeding ${all.length} stadiums...`);

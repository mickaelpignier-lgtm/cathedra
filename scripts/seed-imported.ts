...
import { importedStadiums } from "./generated-seed-data";
...
async function main() {
  ...
  const all: StadiumSeed[] = importedStadiums as any;

  console.log(`Seeding ${all.length} imported stadiums...`);
  ...
  for (const stadium of all) {
    try {
      ...
      console.error(`  ✗ ${stadium.slug}:`, (err as Error).message);
    }
  }
